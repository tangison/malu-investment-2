...
export default function Hero() {
  ...
  useEffect(() => {
    // Parallax effect on scroll - disabled if user prefers reduced motion
  ...
  return (
    <section ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden bg-base">
      ...
      {/* Ticker tape */}
      <div
        className="absolute top-16 sm:top-20 left-0 right-0 z-10 border-y border-amber/20 bg-base/50 backdrop-blur-sm overflow-hidden"
        aria-hidden="true"
      >
      ...
      {/* Content - now full-width 12-col layout with no obsolete teaser panel */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 sm:pt-40 pb-20 w-full">
        <div className="max-w-3xl">
          ...
          {/* Title */}
          <motion.h1
            initial={prefersReducedMotion ? { opacity: 1 } : { y: 60, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.1, duration: 0.7, ease: [0.76, 0, 0.24, 1] }}
            className="font-display text-[clamp(3rem,10vw,8rem)] text-text-primary leading-[0.92] tracking-[0.02em] mb-4"
          >
            MOVING
            <br />
          ...
          <motion.p
            ...
            className="font-[family-name:var(--font-space-grotesk)] text-base sm:text-lg text-text-secondary leading-relaxed mb-8 max-w-xl"
          >
            Taxi transport across Oshakati, Ongwediva, and Ondangwa. Construction,
            logistics, and cleaning coming soon. One company, full Namibia coverage.
          </motion.p>
          ...
          >
            <a
              ...
              className="group flex items-center gap-3 bg-amber text-base font-[family-name:var(--font-space-grotesk)] text-sm uppercase tracking-[0.1em] font-bold px-8 py-4 hover:bg-amber-light transition-all duration-200 min-h-[48px]"
            >
            ...
            <Link
              href="/services"
              className="flex items-center gap-2 border border-base-border text-text-secondary font-[family-name:var(--font-space-grotesk)] text-sm uppercase tracking-[0.1em] font-medium px-8 py-4 hover:border-amber/50 hover:text-amber transition-all duration-200 min-h-[48px]"
            >
              Our Services
            </Link>
          </motion.div>
